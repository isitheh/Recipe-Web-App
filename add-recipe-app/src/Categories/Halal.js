...
class Halal extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const HalalOnlyRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === HALAL_ENUM) {
        //Only add this recipe if it is from the halal category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: HalalOnlyRecipe,
      isLoading: false
    });
  }
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Halal Recipes</h3>
