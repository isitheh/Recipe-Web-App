...
class Gluten extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const GlutenFreeRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === GLUTEN_ENUM) {
        //Only add this recipe if it is from the gluten free category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: GlutenFreeRecipe,
      isLoading: false
    });
  }
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Gluten Free Recipes</h3>
