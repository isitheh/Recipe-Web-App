...
class Fruit extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const fruitOnlyRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === FRUIT_ENUM) {
        //Only add this recipe if it is from the fruit category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: fruitOnlyRecipe,
      isLoading: false
    });
  }
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Fruit Recipes</h3>
