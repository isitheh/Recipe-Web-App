...
class General extends Component {
  ...
  async componentDidMount() {
    const GENERAL_ENUM = 6;
    const responseRecipe = await fetch('http://localhost:8080/api/recipes');
    const bodyRecipe = await responseRecipe.json();
    let stagedRecipes = 0;
    const generalRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === GENERAL_ENUM) {
        //Only add this recipe if it is from the general category.
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View General Recipes</h3>
