...
class Vegan extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const veganOnlyRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === VEGAN_ENUM) {
        //Only add this recipe if it is from the vegan category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: veganOnlyRecipe,
      isLoading: false
    });
  }
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Vegan Recipes</h3>
